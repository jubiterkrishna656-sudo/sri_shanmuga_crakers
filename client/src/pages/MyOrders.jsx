...
import { HiSparkles, HiClock, HiCheckCircle, HiTruck, HiCube, HiXCircle, HiEye, HiArrowRight, HiLocationMarker, HiCreditCard, HiPaperAirplane, HiDeviceMobile, HiSearch } from 'react-icons/hi';
import { orderAPI } from '../utils/api';

const statusConfig = {
  pending: { label: 'Pending', icon: HiClock, color: 'text-yellow-600', bg: 'bg-yellow-50 ring-yellow-200', dot: 'bg-yellow-400', grad: 'from-yellow-400 to-amber-500' },
  payment_verification: { label: 'Payment Verification', icon: HiCreditCard, color: 'text-blue-600', bg: 'bg-blue-50 ring-blue-200', dot: 'bg-blue-400', grad: 'from-blue-400 to-indigo-500' },
  confirmed: { label: 'Confirmed', icon: HiCheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50 ring-emerald-200', dot: 'bg-emerald-400', grad: 'from-emerald-400 to-green-500' },
  packed: { label: 'Packed', icon: HiCube, color: 'text-purple-600', bg: 'bg-purple-50 ring-purple-200', dot: 'bg-purple-400', grad: 'from-purple-400 to-violet-500' },
  shipped: { label: 'Shipped', icon: HiPaperAirplane, color: 'text-indigo-600', bg: 'bg-indigo-50 ring-indigo-200', dot: 'bg-indigo-400', grad: 'from-indigo-400 to-blue-500' },
  delivered: { label: 'Delivered', icon: HiCheckCircle, color: 'text-green-700', bg: 'bg-green-100 ring-green-300', dot: 'bg-green-500', grad: 'from-green-500 to-teal-500' },
  cancelled: { label: 'Cancelled', icon: HiXCircle, color: 'text-red-600', bg: 'bg-red-50 ring-red-200', dot: 'bg-red-400', grad: 'from-red-400 to-rose-500' }
};
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-16">
        <div className="text-center mb-8">
          <div className="w-24 h-24 mx-auto mb-5 rounded-[2rem] bg-gradient-to-br from-orange-100 to-rose-100 flex items-center justify-center shadow-lg">
            <HiDeviceMobile className="text-5xl text-orange-400" />
          </div>
          <h1 className="text-3xl md:text-4xl font-black text-gray-900">
            Track <span className="text-gradient">Orders</span>
          </h1>
          <p className="text-gray-500 mt-2">Enter the phone number you used while placing the order to see live updates.</p>
        </div>
        ...
        <form onSubmit={handleTrack} className="card p-6">
          ...
          <p className="mt-3 text-center text-xs text-gray-400">No login needed. Orders are looked up by phone number.</p>
        </form>
      </div>
    );
  }
          ...
          {orders.map((order, oi) => {
            ...
            return (
              <motion.div key={order._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: oi * 0.05 }}
                className={`card p-6 relative overflow-hidden ${isUpdated ? 'ring-2 ring-orange-300' : ''}`}>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-5 gap-3">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-orange-500 to-pink-500 text-white flex items-center justify-center font-black text-sm shadow-lg shadow-orange-500/25">
                      #{order._id.slice(-4).toUpperCase()}
                    </div>
                    <div>
                      <p className="font-black text-gray-800">Order #{order._id.slice(-8).toUpperCase()}</p>
                  ...
                  <motion.span
                    ...
                    className={`inline-flex items-center gap-1.5 ${statusInfo.bg} ${statusInfo.color} px-4 py-1.5 rounded-full text-sm font-bold ring-1`}
                  >
                      ...
                      return (
                        <div key={s} className="flex items-center flex-1 min-w-0 last:flex-none" title={cfg.label}>
                          <div className="flex flex-col items-center gap-1">
                            <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center transition-all duration-500 ${done ? `bg-gradient-to-br ${statusInfo.grad} text-white shadow-md` : 'bg-gray-100 text-gray-400'} ${isCurrent && isUpdated ? 'scale-125 animate-pulse ring-2 ring-white' : ''}`}>
                          ...
                          {i < statusFlow.length - 1 && (
                            <div className={`flex-1 h-0.5 mx-0.5 md:mx-1 mb-4 rounded ${done ? `bg-gradient-to-r ${cfg.grad}` : 'bg-gray-200'}`} />
                          )}
                        </div>
                      );
                    })}
                ...
                {estDelivery && (
                  <div className="flex items-center gap-2 bg-indigo-50 text-indigo-600 px-4 py-2.5 rounded-2xl text-sm font-semibold ring-1 ring-indigo-100 mb-4">
                ...
                <div className="space-y-2">
                  {order.products.slice(0, expanded === order._id ? undefined : 2).map((p, i) => (
                    <div key={i} className="flex justify-between items-center py-2 border-b border-orange-50 last:border-0">
                      ...
                      <p className="font-black text-sm">₹{p.quantity * p.price}</p>
                    </div>
                  ))}
                  {order.products.length > 2 && (
                    <button onClick={() => setExpanded(expanded === order._id ? null : order._id)} className="text-orange-500 text-sm font-bold hover:text-orange-600">
                      {expanded === order._id ? 'Show less' : `Show all ${order.products.length} items`}
                    </button>
                  )}
                </div>

                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-4 pt-4 border-t border-orange-100 gap-3">
                  {order.address && (
                    <div className="flex items-start gap-2 text-sm">
                      <HiLocationMarker className="text-orange-400 mt-0.5 shrink-0" />
                      <div>
                        <p className="font-bold text-gray-700 text-xs uppercase tracking-wide">Delivery to</p>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-2xl font-black text-gradient">₹{order.totalAmount}</p>
                  </div>
                </div>
                ...
                <AnimatePresence>
                  {order.paymentStatus === 'rejected' && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                      className="mt-4 bg-red-50 text-red-600 px-4 py-3 rounded-2xl text-sm font-semibold ring-1 ring-red-100">
                      Payment rejected. Please contact support.
                    </motion.div>
                  )}
                  {(order.paymentStatus === 'verified' || order.orderStatus === 'delivered') && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                      className="mt-4 bg-green-50 text-green-700 px-4 py-3 rounded-2xl text-sm font-semibold ring-1 ring-green-100 flex items-center gap-2">
                      <HiCheckCircle className="text-base" /> Payment confirmed. Thank you for shopping with us!
                    </motion.div>
                  )}
                  {order.paymentStatus === 'pending' && !['delivered', 'cancelled'].includes(order.orderStatus) && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                      className="mt-4 bg-yellow-50 text-yellow-700 px-4 py-3 rounded-2xl text-sm font-semibold ring-1 ring-yellow-100 flex items-center gap-2">
                      <HiEye className="text-base" /> Payment pending verification. We will update once confirmed.
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
